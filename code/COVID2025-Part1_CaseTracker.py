...
for row in df_cases.itertuples(index=False):
    ...
    if len(long_covid_case_tracker) > 10_000:
        ...
        df_temp.to_csv(os.path.join(base_folder, 'long_covid_cases_popavg_024_031_v14.csv'), mode='a', header=False, index=False)
        long_covid_case_tracker.clear()

    if len(mecfs_cases) > 10_000:
        df_temp = pd.DataFrame(mecfs_cases)
        df_temp.to_csv(os.path.join(base_folder, 'mecfs_cases_popavg_024_031_v14.csv'), mode='a', header=False, index=False)
        mecfs_cases.clear()
